...
function PhotoGallery({ resetTimer }) { 
...
const fetchPhotos = () => { setLoading(true); axios .get("http://localhost:5000/api/photos", { params: { search, date, page } }).then((response) => { setPhotos(response.data.photos); 
setTotal(response.data.total); setLoading(false); }).catch((error) => { console.error("Error fetching data:", error); setLoading(false); }); }; 
...
return ( <div style={{ textAlign: "center", padding: "20px" }}>
 ...
 {photos.map((photo, index) => ( <div key={index} style={{ margin: "20px", maxWidth: "400px" }}> <h3>{photo.title}</h3> 
 ...
 </button> {showExplanations[index] && <p>{photo.explanation}</p>} </div> ))} 
 </div> )} {/* Pagination */} 
